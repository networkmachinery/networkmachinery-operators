...
func newReconciler(mgr manager.Manager) *ReconcileNetworkDelayTest {
	return &ReconcileNetworkDelayTest{
		logger:   log.Log.WithName("networkdelay-test-controller"),
...
func add(mgr manager.Manager, r reconcile.Reconciler, predicates []predicate.Predicate) error {
	...
	if err := ctrl.Watch(&source.Kind{Type: &v1alpha1.NetworkDelayTest{}}, &handler.EnqueueRequestForObject{}, predicates...); err != nil {
		return err
	}
